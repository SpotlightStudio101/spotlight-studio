...
- Contact form (Web3Forms): qualifies leads with a project-type select and an optional budget-range select, alongside the original name/email/message fields
...
- Deploy target: Cloudflare Pages (migrating from Netlify, whose free credit plan capped monthly deploys at ~20). Forms moved from Netlify Forms to Web3Forms (free, public access key in `src/data/site.ts`) since Cloudflare has no built-in form handling
